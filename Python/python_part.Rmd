...
Then, the rows containing missing values are deleted.
...
plt.show()
```
![Caption for the picture.](scree.png)
...
In conclusion, five is the most reasonable number of factors. With this number of factors, the columns that are originally correlated would be mostly accounted by the same factor, meaning that the structure of the original data is successfully figured out by factor analysis. 
