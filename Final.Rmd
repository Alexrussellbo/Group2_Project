...
# Things to consider
 + Based on the final factor loadings obtained by three aproaches, they are nearly the same but it seems that the variables has much dependence on factors using Stata than R and Python. Later we may figure out the reasons why it happens.
 + We use almost the same methods to determining the number of factors in three ways, but there are different results coming out. Fortunately, our final best number of factors is included in all three sets of results. We hope to consider further why the difference comes out, especially the difference from scree plot between R and Python.
 
 + We all conduct factor analysis by "varimax" rotation method. What if we utilize other kinds of rotation, such as "quartimax" , "bentlerT", "equamax", etc. Do they change the results dramatically or not?
...
#Reference
  ...
  + Phil Ender. Statistical Consulting Group.  UCLA: [Academic Technology Services](https://stats.idre.ucla.edu/stat/stata/ado/analysis)
